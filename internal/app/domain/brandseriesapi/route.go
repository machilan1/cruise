package brandseriesapi

import (
	"net/http"

	"github.com/machilan1/cruise/internal/app/sdk/mid"
	"github.com/machilan1/cruise/internal/business/domain/auth"
	"github.com/machilan1/cruise/internal/business/domain/brandseries"
	"github.com/machilan1/cruise/internal/business/sdk/sess"
	"github.com/machilan1/cruise/internal/business/sdk/tran"
	"github.com/machilan1/cruise/internal/framework/logger"
	"github.com/machilan1/cruise/internal/framework/web"
)

type Config struct {
	Log         *logger.Logger
	TxM         tran.TxManager
	Sess        *sess.Manager
	Auth        *auth.Core
	BrandSeries *brandseries.Core
}

func Routes(app *web.App, cfg Config) {
	const version = "v1"

	bsCtx := brandSeriesCtx(*cfg.BrandSeries)

	hdl := newHandlers(cfg.Log, cfg.TxM, cfg.BrandSeries)

	authn := mid.AuthN(cfg.Sess, cfg.Auth)
	authAdmin := mid.AuthNAndIsOneOfAdmin(cfg.Sess, cfg.Auth)

	authApp := *app
	authApp.PostMid = []web.MidFunc{
		authn,
	}
	authIsOneofAdmin := *app
	authIsOneofAdmin.PostMid = []web.MidFunc{
		authAdmin,
	}

	authApp.HandleFunc(http.MethodGet, version, "/brand-series", hdl.query)
	authApp.HandleFunc(http.MethodGet, version, "/brand-series/{brandSeriesID}", hdl.queryByID, bsCtx)

	authIsOneofAdmin.HandleFunc(http.MethodPost, version, "/brand-series", hdl.create)
	authIsOneofAdmin.HandleFunc(http.MethodPut, version, "/brand-series/{brandSeriesID}", hdl.update, bsCtx)
	authIsOneofAdmin.HandleFunc(http.MethodDelete, version, "/brand-series/{brandSeriesID}", hdl.delete, bsCtx)
}
